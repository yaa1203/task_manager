...
<div class="max-w-6xl mx-auto">
    ...
    <div class="mb-8">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                ...
                <p class="text-sm sm:text-base text-gray-600">Kelola kategori yang dapat dipilih oleh pengguna dan admin</p>
            </div>
            <div>
                <a href="{{ route('categories.create') }}"
                   class="inline-flex items-center gap-2 px-5 py-3 bg-gradient-to-r from-purple-600 to-purple-700 text-white rounded-xl hover:from-purple-700 hover:to-purple-800 transition-all font-semibold text-sm shadow-lg hover:shadow-xl">
        ...
        <div class="mb-6 p-4 bg-purple-50 border-l-4 border-purple-500 rounded-lg mt-6">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xs text-purple-700 mt-0.5">Menampilkan semua kategori dari seluruh sistem</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-gray-200 shadow-sm p-4 sm:p-5 mb-6">
        <form method="GET" action="{{ route('categories.index') }}" class="flex flex-col sm:flex-row gap-3">
            ...
            <div class="flex-1 flex gap-2">
                ...
                {{-- Tombol Reset --}}
                @if($search)
                <a href="{{ route('categories.index', ['sort_by' => $sortBy]) }}" 
            ...
            <div class="flex gap-2">
                <select name="sort_by" onchange="this.form.submit()" 
                    ...
                    <option value="admin_count" {{ $sortBy == 'admin_count' ? 'selected' : '' }}>Admin (Terbanyak)</option>
                    <option value="user_count" {{ $sortBy == 'user_count' ? 'selected' : '' }}>User (Terbanyak)</option>
                </select>
            </div>
        </form>
    </div>
                ...
                @if($sortBy != 'created_at')
                    <span class="inline-flex items-center px-2 py-1 rounded-md text-xs font-medium bg-purple-100 text-purple-800 ml-2">
                        @switch($sortBy)
                            ...
                            @case('admin_count') Admin (Terbanyak) @break
                            @case('user_count') User (Terbanyak) @break
                        @endswitch
                    </span>
                @endif
            </p>
        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden hover:shadow-md transition-shadow">
            <div class="p-6">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <span class="px-3 py-1 bg-blue-50 text-blue-700 text-xs font-semibold rounded-full">Admin</span>
                </div>
                <h3 class="text-sm font-medium text-gray-600 mb-1">Total Admin</h3>
                <p class="text-3xl font-bold text-gray-900">{{ $categories->sum('admin_count') }}</p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden hover:shadow-md transition-shadow">
            <div class="p-6">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <span class="px-3 py-1 bg-green-50 text-green-700 text-xs font-semibold rounded-full">User</span>
                </div>
                <h3 class="text-sm font-medium text-gray-600 mb-1">Total User</h3>
                <p class="text-3xl font-bold text-gray-900">{{ $categories->sum('user_count') }}</p>
            </div>
        </div>
    </div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gradient-to-r from-purple-50 to-purple-100">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-center text-xs font-bold text-gray-700 uppercase tracking-wider">Jumlah Admin</th>
                        <th class="px-6 py-4 text-center text-xs font-bold text-gray-700 uppercase tracking-wider">Jumlah User</th>
                ...
                <tbody class="bg-white divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors duration-150">
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 rounded-xl bg-gradient-to-br from-purple-500 to-purple-600 flex items-center justify-center flex-shrink-0 shadow-md">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-center">
                            <span class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold bg-green-50 text-green-700 border border-green-200">
                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"/>
                                </svg>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                            <div class="flex items-center gap-2">
                                ...
                                <form action="{{ route('categories.destroy', $category) }}" method="POST"
                                      onsubmit="return confirm('Apakah Anda yakin ingin menghapus kategori ini? Semua admin dan user dengan kategori ini akan kehilangan kategorinya.');">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
                    ...
                    <tr>
                        <td colspan="6" class="px-6 py-20 text-center">
                            <div class="flex flex-col items-center justify-center">
                                ...
                                <p class="text-sm text-gray-600 max-w-md mb-4">Tidak ada kategori yang ditemukan. Tambahkan kategori baru untuk memulai.</p>
                                <a href="{{ route('categories.create') }}"
                                   class="inline-flex items-center gap-2 px-5 py-2.5 bg-purple-600 text-white rounded-xl hover:bg-purple-700 transition-all font-semibold text-sm shadow-sm">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/>
                                    </svg>
    ...
    <div class="lg:hidden space-y-4">
            ...
            <div class="p-5">
                ...
                <div class="flex items-start gap-3 mb-4">
                    ...
                    <div class="flex-1 min-w-0">
                        <h3 class="text-base font-bold text-gray-900 mb-1">{{ $category->name }}</h3>
                        <p class="text-xs text-gray-600 mb-3">{{ $category->description ?? 'Tidak ada deskripsi' }}</p>
                        <div class="flex items-center gap-2 flex-wrap">
                ...
                {{-- Additional Info --}}
                <div class="mb-4 bg-gradient-to-br from-purple-50 to-purple-100 rounded-xl p-3 border border-purple-200">
                    <div class="flex items-center gap-2 text-xs text-gray-700">
                        <svg class="w-4 h-4 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/>
                        </svg>
                        <span class="text-gray-600">Dibuat:</span>
                        <span class="font-semibold text-gray-900">{{ $category->created_at->format('d F Y') }}</span>
                ...
                <div class="flex gap-2">
                    ...
                    <form action="{{ route('categories.destroy', $category) }}" method="POST"
                          onsubmit="return confirm('Apakah Anda yakin ingin menghapus kategori ini?');" class="flex-1">
                        @csrf
                        @method('DELETE')
                        <button type="submit"
                                class="w-full inline-flex items-center justify-center gap-2 px-4 py-3 bg-rose-600 text-white rounded-xl hover:bg-rose-700 transition-all font-semibold text-sm shadow-sm hover:shadow-md">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/>
                            </svg>
                            Hapus
        ...
        <div class="bg-white rounded-2xl border border-gray-200 shadow-sm p-16 text-center">
            ...
            <h3 class="text-lg font-bold text-gray-900 mb-2">Belum Ada Kategori</h3>
            <p class="text-sm text-gray-600 max-w-md mx-auto mb-4">Tidak ada kategori yang ditemukan. Tambahkan kategori baru untuk memulai.</p>
            <a href="{{ route('categories.create') }}"
               class="inline-flex items-center gap-2 px-5 py-2.5 bg-purple-600 text-white rounded-xl hover:bg-purple-700 transition-all font-semibold text-sm shadow-sm">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/>
