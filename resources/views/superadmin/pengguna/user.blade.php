...
<div class="max-w-6xl mx-auto">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-8">
        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden hover:shadow-md transition-shadow">
            <div class="p-6">
                ...
                <p class="text-3xl font-bold text-gray-900">
                    {{ \App\Models\User::where('role', 'user')->where('is_blocked', false)->count() }}
                </p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden hover:shadow-md transition-shadow">
            <div class="p-6">
                ...
                <h3 class="text-sm font-medium text-gray-600 mb-1">User Diblokir</h3>
                <p class="text-3xl font-bold text-gray-900">
                    {{ \App\Models\User::where('role', 'user')->where('is_blocked', true)->count() }}
                </p>
            </div>
        </div>
                    ...
                    <tr class="hover:bg-gray-50 transition-colors duration-150 {{ $user->is_blocked ? 'bg-red-50' : '' }}">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex flex-col gap-2">
                                ...
                                class="flex items-center gap-2 text-sm text-green-600 hover:text-green-700 hover:underline transition-colors group">
                                    ...
                                    <span class="truncate max-w-[180px] font-medium">{{ $user->whatsapp }}</span>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                            <div class="flex items-center gap-2">
                                @if($user->is_blocked)
                                <form action="{{ route('pengguna.user.unblock', $user->id) }}" method="POST" class="inline">
                                    @csrf
                                ...
                                @else
                                <form action="{{ route('pengguna.user.block', $user->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin memblokir user {{ $user->name }}? User tidak akan bisa login ke sistem.');">
                                    @csrf
    ...
    <div class="lg:hidden space-y-4">
        @forelse($users as $user)
        <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden hover:shadow-lg transition-all duration-300 {{ $user->is_blocked ? 'bg-red-50 border-red-200' : '' }}">
            <div class="p-5">
                {{-- Header --}}
                <div class="flex items-start gap-3 mb-4">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $user->whatsapp) }}" 
                        target="_blank"
                        class="flex items-center gap-2 text-xs text-green-600 hover:text-green-700 mb-2 group">
                            <svg class="w-3.5 h-3.5 flex-shrink-0 group-hover:scale-110 transition-transform" fill="currentColor" viewBox="0 0 24 24">
                                <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z"/>
                            </svg>
                            <span class="truncate font-semibold">{{ $user->whatsapp }}</span>
                        ...
                        <div class="flex items-center gap-2 text-xs text-gray-400 mb-2">
                            ...
                            <span class="italic">Tidak ada WhatsApp</span>
                        </div>
                        @endif
                ...
                <div class="flex gap-2 flex-wrap">
                    @if($user->is_blocked)
                    <form action="{{ route('pengguna.user.unblock', $user->id) }}" method="POST" class="flex-1">
                        @csrf
                    ...
                    @else
                    <form action="{{ route('pengguna.user.block', $user->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin memblokir user {{ $user->name }}? User tidak akan bisa login ke sistem.');" class="flex-1">
                        @csrf
